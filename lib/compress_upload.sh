...
set -euo pipefail # Exit on unhandled error, unbound variable, pipe failure
...
# With set -o pipefail enabled, $? will reflect failure of any command in the above
# pipeline, not just the aws command
if [ $? -ne 0 ]; then
